...
data_LIVEVQC = load(...
    'D:\JAVERIANA\THESIS\RESULTS\BRISQUE\Qualcomm\Features\Matriz_features_full_videos_qualcomm.mat');

% Se cargan los puntajes subjetivos 
MOS_LIVEVQC =  load(...
    'D:\JAVERIANA\THESIS\MODIFICACION DE MATRICES\BRISQUE\mos_qualcomm_todo_brisque.mat');
data_LIVEVQC=data_LIVEVQC.matriz_videos_features;
MOS_LIVEVQC = MOS_LIVEVQC.mos_unbiased;

for i=1:1
    i
% Se generan matrices con los puntajes de cada de las distorciones aleatoreamente
    matriz_1_Artefacts =data_LIVEVQC(1:35,:);
    mos_Artefacts=MOS_LIVEVQC(1:35,:);
    matriz_2_Color = data_LIVEVQC(36:70,:);
    mos_Color=MOS_LIVEVQC(36:70,:);
    matriz_3_Exposure = data_LIVEVQC(71:104,:);
    mos_Exposure=MOS_LIVEVQC(71:104,:);
    matriz_4_Focus = data_LIVEVQC(105:138,:);
    mos_Focus=MOS_LIVEVQC(105:138,:);
    matriz_5_Sharpness = data_LIVEVQC(139:172,:);
    mos_Sharpness=MOS_LIVEVQC(139:172,:);
    matriz_6_Stabilization = data_LIVEVQC(173:207,:);
    mos_Stabilization=MOS_LIVEVQC(173:207,:);

    
    [trainInd,valInd,testInd] = dividerand(34,Training_Percentage,0,Test_Percentaje);     
    Training_DataC1 = matriz_1_Artefacts(trainInd,:);
    Training_MOSC1 = mos_Artefacts (trainInd);
    Test_DataC1 = matriz_1_Artefacts(testInd,:);
    Test_MOSC1 = mos_Artefacts (testInd);
    
    [trainInd,valInd,testInd] = dividerand(34,Training_Percentage,0,Test_Percentaje);      
    Training_DataC2 = matriz_2_Color(trainInd,:);
    Training_MOSC2 = mos_Color (trainInd);
    Test_DataC2 = matriz_2_Color(testInd,:);
    Test_MOSC2 = mos_Color (testInd);
    
    [trainInd,valInd,testInd] = dividerand(34,Training_Percentage,0,Test_Percentaje);  
    Training_DataC3 = matriz_3_Exposure(trainInd,:);
    Training_MOSC3 = mos_Exposure (trainInd);
    Test_DataC3 = matriz_3_Exposure(testInd,:);
    Test_MOSC3 = mos_Exposure (testInd);
